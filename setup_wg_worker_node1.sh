...
echo "[w1] Installing WireGuard..."
sudo apt-get update -y
sudo apt-get install -y wireguard

echo "[w1] Generating keys (if missing)..."
umask 077
...
if [[ "$CP1_PUBKEY" == PASTE_* ]]; then
  echo "[w1] ERROR: You must paste CP1_PUBKEY into this script first."
  exit 1
fi

echo "[w1] Writing /etc/wireguard/wg0.conf ..."
